...
def __fix_line(line, cols):
        ...
        if(line[:cols - 1].rstrip() == ""): # when all of characters before index are whitespace
                ...
                # Note: this condition compares the last character of the string stripped whitespaces, that the string is produced by slicing until one letter before the specified character.
                # this means that it is missing the double-quote character in the after part from specified character.
                ...
                # this means that it is missing the double-quote character in the former part from specified character.
...
def insert_before_zenkaku_str(lines, params):
        # correct column number which includes Zenkaku characters
        cols = correct_cols(lines[int(params[0]) - 1], int(params[1]))

        # complement the string literal by inserting the " character to appropriate position before the index received in parameter
        lines[int(params[0]) - 1] = insert_quote_backward(lines[int(params[0]) - 1], cols, '"')
        return lines