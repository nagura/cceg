...
def compile_process(src, exec, warn, cwd=None, display=False):
...
def display_group(g_no, msg_grps, list_errmsg):
        ...
        if g_msg_list != []:
                ...
                if g_no == 0:
                        # g_no == 0 は，分類不能なメッセージ
                        print("--- Uncategorized Message ---")
                else:
                        print("--- Error Group #" + str(g_no) + " ---")
                ...
                for [msg, func_name] in list_errmsg:
                        ...
                        if src_fname != None:
                                if the_func != func_name:
                                       the_func = func_name
                                       print(src_fname + ": In function " + the_func + ":")
                                print(msg)
...
def main():
        ...
        ### 元のソースファイルのコンパイル
        if store_msgs(compile_process(src, exec, warn, display=disp), tmpdir, out_fname) == False:
...
if __name__ == "__main__":
        ### コマンドライン引数の解析
        parser = argparse.ArgumentParser(description="C Compiler with Error message Grouping")
